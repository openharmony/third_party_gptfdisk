...
MbrResult OhosFixparts::ParsePartNum(const std::string &str, int &partitionNum)
{
    ...
    if (str.empty()) {
        std::cerr << "Invalid partition number: NULL" << std::endl;
        return MbrResult::ERROR_INVALID_PARTITION;
    }
    ...
    if (iss.fail() || !iss.eof()) {
        std::cerr << "Parse partNum fail: " << std::to_string(iss.fail()) << "/"
                  << std::to_string(iss.eof()) << " str: " << str.c_str() << std::endl;
        return MbrResult::ERROR_INVALID_PARTITION;
    }
...
bool OhosFixparts::ParseTypeCode(const std::string &str, uint8_t &code)
{
    ...
    if (iss.fail() || !iss.eof()) {
        std::cerr << "Parse typecode fail: " << std::to_string(iss.fail()) << "/"
                  << std::to_string(iss.eof()) << " str: " << str.c_str() << std::endl;
        return false;
    }
    ...
    if (value < 0x01 || value > 0xFF) {
        std::cerr << "Invalid type code(must be 0x01-0xFF): " << value << std::endl;
        return false;
    }
